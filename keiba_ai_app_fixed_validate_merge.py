...
def apply_style_ground_pace_adjustments(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["ai_score_base"] = pd.to_numeric(out.get("ai_score_base", out.get("ai_score", 0)), errors="coerce").fillna(0)
    out["ai_score"] = out["ai_score_base"] + pd.to_numeric(out["style_bonus"], errors="coerce").fillna(0)
    ...
    out["stability_score"] = (
        show_rate_col * 0.35
        + top2_rate_col * 0.25
        + pd.to_numeric(out.get("s_recent3", 0), errors="coerce").fillna(0) * 0.20
        + pd.to_numeric(out.get("s_last3f", 0), errors="coerce").fillna(0) * 0.20
    )
    out["win_score"] = (
        out["win_prob"].fillna(0) * 0.55
        + out["gap"].fillna(0) * 0.25
        + pd.to_numeric(out.get("s_last3f", 0), errors="coerce").fillna(0) * 0.10
        + pd.to_numeric(out.get("jockey_score", 0), errors="coerce").fillna(0) * 0.05
        + pd.to_numeric(out.get("style_bonus", 0), errors="coerce").fillna(0) * 0.05
    )

    out = out.sort_values(["win_score", "ai_score", "win_prob"], ascending=False).reset_index(drop=True)
    marks = ["◎", "○", "▲", "△", "☆"]
    out["mark"] = ""
...
def analyze_race(race: pd.DataFrame, hist: pd.DataFrame, recent_n: int = 10) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, bool]:
    ...
    df["ai_score_base"] = pd.to_numeric(df["ai_score"], errors="coerce").fillna(0)
    df = apply_style_ground_pace_adjustments(df)

    top = df.head(min(12, len(df))).copy()
    honmei = df.sort_values(["win_score", "win_prob", "ai_score"], ascending=False).head(3).copy()
    ana = df.sort_values(["ana_score", "gap"], ascending=False).head(8).copy()
    return df, top, honmei, ana, pd.to_numeric(df["odds_f"], errors="coerce").notna().sum() > 0
...
def render_results(race: pd.DataFrame, hist: pd.DataFrame, recent_n: int, ana_count: int, ticket_head_count: int, wide_count: int, umaren_count: int, trio_count: int, trifecta_count: int) -> None:
    ...
    display_cols = ["mark", "horse_no", "frame_no", "horse_name", "running_style", "ai_score", "win_score", "win_prob", "place_prob", "ana_score"]
    if odds_valid:
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "win_score": st.column_config.NumberColumn("Winスコア", format="%.4f", disabled=True),
    ...
    st.subheader("本命候補")
    st.dataframe(honmei[["mark", "horse_no", "horse_name", "win_score", "win_prob", "place_prob", "ai_score", "running_style"]], use_container_width=True, hide_index=True)
