...
# 数値列安全化パッチ v3
# ============================================================
NUMERIC_FORCE_COLUMNS = [
    "horse_no", "frame_no", "odds", "odds_num", "odds_f",
    "popularity", "pop_num", "pop_f", "finish", "finish_num",
    "last3f", "last3f_num", "last3f_avg", "corner_num",
    "hist_odds_avg", "hist_pop_avg", "ai_score", "win_prob",
    "place_prob", "top3_prob", "top5_prob", "market_prob",
    "gap", "ana_score", "ev_tansho", "jockey_score",
    "show_rate", "top2_rate", "quinella_rate", "win_rate",
    "recent3_avg", "recent5_avg", "recent3_best", "last_finish",
    "std_finish", "median_finish", "growth", "nakayama_score",
    "model_top3_prob", "model_rank_pred", "model_value_score",
]

def _coerce_numeric_text_series(s: pd.Series) -> pd.Series:
    ...
    txt = txt.str.replace("倍", "", regex=False)
    txt = txt.str.replace("番人気", "", regex=False)
    txt = txt.str.replace("人気", "", regex=False)
    txt = txt.str.replace("kg", "", regex=False)
    txt = txt.str.replace("㎏", "", regex=False)
    extracted = txt.str.extract(r"(-?\d+(?:\.\d+)?)", expand=False)
    return pd.to_numeric(extracted, errors="coerce")

def force_numeric_columns(df: Any, cols: Optional[List[str]] = None) -> pd.DataFrame:
    out = _safe_df(df) if '_safe_df' in globals() else (df.copy() if isinstance(df, pd.DataFrame) else pd.DataFrame(df))
    for c in (cols or NUMERIC_FORCE_COLUMNS):
        if c in out.columns:
            out[c] = _coerce_numeric_text_series(out[c])
    return out

def safe_ge(series: Any, threshold: Any, fill: float = np.nan) -> pd.Series:
    s = _coerce_numeric_text_series(series if isinstance(series, pd.Series) else pd.Series(series))
    t = pd.to_numeric(pd.Series([threshold]), errors="coerce").iloc[0]
    return s.fillna(fill) >= t

def safe_le(series: Any, threshold: Any, fill: float = np.nan) -> pd.Series:
    s = _coerce_numeric_text_series(series if isinstance(series, pd.Series) else pd.Series(series))
    t = pd.to_numeric(pd.Series([threshold]), errors="coerce").iloc[0]
    return s.fillna(fill) <= t

def safe_gt(series: Any, threshold: Any, fill: float = 0.0) -> pd.Series:
    s = _coerce_numeric_text_series(series if isinstance(series, pd.Series) else pd.Series(series))
    t = pd.to_numeric(pd.Series([threshold]), errors="coerce").iloc[0]
    return s.fillna(fill) > t
...
def apply_style_ground_pace_adjustments(df: pd.DataFrame) -> pd.DataFrame:
    out = force_numeric_columns(df.copy())
...
def analyze_race(race: pd.DataFrame, hist: pd.DataFrame, recent_n: int = 10, use_jockey_score: bool = True, local_mode: str = "自動", predict_mode: str = "バランス") -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, bool]:
    ...
    if pd.to_numeric(df["pop_f"], errors="coerce").notna().sum() > 0:
        df.loc[safe_ge(df["pop_f"], 5, fill=np.inf) & safe_le(df["pop_f"], 10, fill=-np.inf), "ai_score"] += 0.02
    if pd.to_numeric(df["frame_no"], errors="coerce").notna().sum() > 0:
        frame_s = _coerce_numeric_text_series(df["frame_no"])
        df.loc[(df["running_style"] == "先行") & (frame_s <= 3), "ai_score"] += 0.02
        df.loc[(df["running_style"] == "先行") & (frame_s >= 6), "ai_score"] -= 0.015
        df.loc[(df["running_style"] == "差し") & (frame_s >= 6), "ai_score"] += 0.015
    ...
    if odds_valid:
        ...
        df["ev_tansho"] = pd.to_numeric(df["win_prob"], errors="coerce").fillna(0) * pd.to_numeric(df["odds_f"], errors="coerce").fillna(0) * 0.8
    else:
        if pd.to_numeric(df["pop_f"], errors="coerce").notna().sum() > 0:
...
def select_honmei_and_ana(
    ...
    work = force_numeric_columns(df.copy())
    if len(work) == 0:
...
def render_results(race: pd.DataFrame, hist: pd.DataFrame, recent_n: int, ana_count: int, ticket_head_count: int, wide_count: int, umaren_count: int, trio_count: int, trifecta_count: int) -> None:
    ...
    df = force_numeric_columns(dedupe_race_df(df))
    ...
    df = force_numeric_columns(apply_style_ground_pace_adjustments(df))
